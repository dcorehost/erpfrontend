...
const UpdateUserProfile = () => {
  ...
  const fetchProfileUpdateRequests = async () => {
    try {
      ...
      const axiosInstance = getAxiosInstance();
      const response = await axiosInstance.get('get-user-profile-update-request');
      const safeRequests = (response.data.requests || []).map(r => ({
        ...r,
        updates: r.updates || {} // Ensure updates is never null/undefined
      }));
  ...
  const handleProcessRequest = async () => {
    ...
    try {
      ...
      if (action === 'reject') payload.remarks = remarks;
  ...
  if (error) {
    return (
      <div className={styles.errorContainer}>
        <div className={styles.errorIcon}>!</div>
        <p>{error}</p>
        <button onClick={fetchProfileUpdateRequests} className={styles.retryButton}>Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.headerContent}>
          <h1><i className="fas fa-user-cog"></i> Profile Update Requests</h1>
        </div>
        <div className={styles.headerActions}>
          <button onClick={fetchProfileUpdateRequests} className={styles.refreshButton}>
            <i className="fas fa-sync-alt"></i> Refresh
          ...
          <div className={styles.requestsGrid}>
            <div className={styles.requestsList}>
              {requests.map((request) => (
                <div
                  key={request._id}
                  className={`${styles.requestCard} ${selectedRequest?._id === request._id ? styles.selected : ''}`}
                  onClick={() => setSelectedRequest(request)}
                >
                  <div className={styles.cardHeader}>
                    <div className={styles.userInfo}>
                      <div className={styles.avatar}>
                        {request.userId?.displayName?.charAt(0) || request.userId?.username?.charAt(0) || 'U'}
                      </div>
                      <div>
                        <h4>{request.userId?.displayName || 'Unknown User'}</h4>
                        <p>@{request.userId?.username || 'unknown'}</p>
            ...
            <div className={styles.actionPanel}>
              {selectedRequest ? (
                <>
                  <div className={styles.panelHeader}>
                    <h3>Process Request</h3>
                    <button className={styles.closePanel} onClick={() => setSelectedRequest(null)}>
                      <i className="fas fa-times"></i>
                    </button>
                  </div>
                  ...
                  <div className={styles.requestSummary}>
                    <div className={styles.summaryUser}>
                      <div className={styles.avatarLarge}>
                        {selectedRequest.userId?.displayName?.charAt(0) ||
                         selectedRequest.userId?.username?.charAt(0) || 'U'}
                      </div>
                      <div>
                        <h4>{selectedRequest.userId?.displayName || 'Unknown User'}</h4>
                        <p>@{selectedRequest.userId?.username || 'unknown'}</p>
