...
//           { url: 'http://209.74.89.83/erpbackend/get-submittedTasks-forUser', status: 'submitted' },
//           { url: 'http://209.74.89.83/erpbackend/get-completedTasks-forUser', status: 'completed' }
//         ];
...
//         submitted: submittedResponse.data?.taskDetails?.map(task => ({
//           id: task._id,
//           name: task.taskName,
//           description: task.description,
//           project: task.projectName,
...
//         completed: completedResponse.data?.taskDetails?.map(task => ({
//           id: task._id,
//           name: task.taskName,
//           description: task.description,
//           project: task.projectName,
...
const TaskStatus = () => {
  ...
  const columnDisplayNames = {
    ...
    submittedForReview : 'Submitted For Review',  // ✅ Match with backend
    completed: 'Completed'
  };
  ...
  const handleDrop = async (e, targetColumn) => {
    ...
    try {
      ...
      // After successfully updating, fetch all tasks again to ensure they appear in correct columns
      await fetchAllTasks();
      toast.success('Task status updated successfully');
    } catch (error) {
      // Rollback on error
